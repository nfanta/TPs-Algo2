...
\begin{Representacion}
  ...
  \Titulo{Representación de DiccClavesRapidas}
  ...
  \item Todo nodo contenido en la estructura posee el arreglo $sig$ con tamaño igual a 256
...
\begin{Algoritmos}
  ...
  \begin{algorithm}[H]
    ...
    \KwData{O(Longitud(n))}
    \KwComm{$AgregarRapido(n, e.claves)$ es O(1) y se puede utilizar dado que se sabe por el if que no esta definido. El it se guarda para luego poder eliminar del conjunto en O(1). En la rama else, donde la clave ya había sido definida anteriormente, solo actualizamos el significado.}
