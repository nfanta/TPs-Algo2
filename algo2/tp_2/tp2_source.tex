...
\begin{Representacion}
  ...
  \Rep[estr][e]{
  ($\forall$ $p$ : pos) $p$ $\in$ campus.obstaculos $\implies$ ( $\Pi_{1}$($p$) $\leq$ c.alto $\land$ $\Pi_{2}$($p$) $\leq$ c.ancho )
  }\mbox{}

  
  \AbsFc[estr]{campus}[e]{c : campus / \\
    obstaculos($c$) $\igobs$ e.obstaculos $\land$
    alto($c$) $\igobs$ e.alto $\land$
    ancho($c$) $\igobs$ e.ancho \\}

  Segunda version (con observadores)
  \newline

  \AbsFc[estr]{campus}[e]{c : campus / \\
    ($\forall$ $p$ : pos) ocupada?($p, c$) $\igobs$ ocupada?($p, e$) $\land$
    alto($c$) $\igobs$ e.alto $\land$
    ancho($c$) $\igobs$ e.ancho}
