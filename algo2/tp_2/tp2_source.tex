...
\textbf{Algoritmos de Agentes}
  ...
  \begin{algorithm}[H]
    ...
    \Begin{
      ...
      Obtener(a, as).sanciones $\leftarrow$ (Obtener(a, as).sanciones + 1) \hfill \textbf{//O(1)} \\
      as.huboSanciones $\leftarrow$ true \hfill \textbf{//O(1)} \\
      {\small //Ahora tengo que modificar mismSanciones para que refleje el cambio \\ }
      iteradorLista $\leftarrow$ Obtener(a, as).conMismSanciones \hfill \textbf{//O(1)} \\
      ...
      \eIf(\hfill \textbf{//O(1)}){$\neg$ HaySiguiente(iteradorLista)}
        ...
        {
        \eIf(\hfill \textbf{//O(1)}){Obtener(DameUno(Siguiente(iteradorLista)), as).sanciones $>$ Obtener(a, as).sanciones}
          {
      ...
      Obtener(a, as).conMismSanciones $\leftarrow$ iteradorLista \hfill \textbf{//O(1)}
    }
  ...
  \begin{algorithm}[H]
    \text{$i$CambiarPosicion(\In{a}{nat}, \In{p}{posicion}, \Inout{as}{agentes}) $\rightarrow$ res: estr}
  
    \Begin{
      Obtener(a, as).posicion $\leftarrow$ p \hfill \textbf{//O(1)} \\
    }
    \KwData{O(1)}
  ...
  \begin{algorithm}[H]
    \text{$i$AgregarCaptura(\In{a}{nat}, \Inout{as}{agentes}) $\rightarrow$ res: estr}
  
    \Begin{
      Obtener(a, as).capturas $\leftarrow$ (Obtener(a, as).capturas + 1) \hfill \textbf{//O(1)} \\
      {\small //Ademas de sumar captura, hago el mantenimiento de masVigilante  \\}
      \eIf(\hfill \textbf{//O(1)}){Obtener(a, as).capturas $>$ Obtener(as.masVigilante, as).capturas}
        {as.masVigilante $\leftarrow$ a \hfill \textbf{//O(1)}}
        {
          \If(\hfill \textbf{//O(1)}){Obtener(a, as).capturas $==$ Obtener(as.masVigilante, as).capturas}
          {
