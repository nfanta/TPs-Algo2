...
\begin{Representacion}
  ...
  \begin{Estructura}{agentes}[estr]
    \begin{Tupla}[estr]
      \tupItem{info}{DiccRapido(datos)}%
      \tupItem{masVig}{nat}%
      \tupItem{huboSanciones}{bool}%
      \tupItem{mismSanciones}{ListaEnlazada(NodoAgente)}%
    ...
    \begin{Tupla}[datos]
      ...
      \tupItem{pos}{posicion}%
      \tupItem{capturas}{nat}%
      \tupItem{conMismSanciones}{Iterador(ListaEnlazada)}%
    \end{Tupla}

    \begin{Tupla}[NodoAgente]
      \tupItem{mismos}{Conj(nat)}%
      \tupItem{sig}{Puntero(NodoAgente)}%
    \end{Tupla}
    ...
    {\small La idea de la lista enlazada mismSanciones es que guarde en cada nodo a todos aquellos agentes que comparten sanciones, con rapido acceso gracias al Iterador en los datos del agente. El arreglo kSanciones se utiliza para ordenar a los agentes por su cantidad de sanciones en tiempo O(N), y poder buscar a alguno con $K$ sanciones para acceder a aquellos que tienen la misma cantidad via mismSanciones}
