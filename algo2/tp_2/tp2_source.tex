...
\begin{Interfaz}
  ...
  \textbf{usa}: \TipoVariable{conjRapido($\alpha$)}, \TipoVariable{diccRapido(clave, significado)}, \TipoVariable{nat}, \TipoVariable{bool}, \TipoVariable{lista}
  ...
  \InterfazFuncion{ConMismasSanciones}{\In{a}{placa}, \In{as}{agentes}}{conjRapido(placa)}%
  [estaAgente($a, as$)]
...
\begin{Representacion}
  ...
  \begin{Estructura}{agentes}[estr]
    \begin{Tupla}[estr]
      ...
      \tupItem{claves}{conjRapido(placa)}
      \tupItem{masVig}{placa}%
      \tupItem{huboSanciones}{bool}%
      \tupItem{mismSanciones}{lista(conjRapido(placa))}%
    ...
    \begin{Tupla}[datos]
      ...
      \tupItem{capturas}{nat}%
      \tupItem{conMismSanciones}{itLista(conjRapido(placa))}%
    \end{Tupla}
  ...
  \begin{enumerate}
  \item ( ($\forall$ $a$ : nat) def?($a$, e.as) $\iff$ En($a$, e.claves) ) $\yluego$
