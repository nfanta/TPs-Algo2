...
\begin{Representacion}
  ...
  \item Si no hubo sanciones ($\neg$huboSanciones), entonces el arreglo kSanciones representa a los agentes en orden creciente de sanciones.
  ...
  \item ( ($\forall$ $a$ : nat) def?($a$, e.as) $\impluego$ ($\exists!$ $i$ : nat) e.kSanciones[$i$] $=$ $a$  $\land$
  \item $\neg$(e.huboSanciones) $\implies$ SancionesOrdenadas(e) $\land$
  ...
  \tadAxioma{SancionesOrdenadas($e$)}{($\forall$ $i$ : nat) $i$ $<$ (Tam(e.kSanciones)$-1$) $\impluego$ Obtener(e.kSanciones[$i$], e.as).sanciones $\leq$ Obtener(e.kSanciones[$i+1$], e.as).sanciones}
  ~      
...
\textbf{Algoritmos de Agentes}
  ...
  \begin{algorithm}[H]
    \text{$i$NuevoAgentes(\In{d}{DiccRapido(placa,datos)}) $\rightarrow$ res: estr}
  ...
  \begin{algorithm}[H]
    \text{$i$Agentes?(\In{as}{estr}) $\rightarrow$ res: itConj}
  ...
  \begin{algorithm}[H]
    \text{$i$ConKSanciones(\In{k}{nat}, \In{as}{estr}) $\rightarrow$ res: puntero(conj(nat))}
    ...
    \Begin{
      ...
      \If(\hfill \textbf{//O(1)}){as.huboSanciones == true}
        {
          CountingSortSanciones(as.kSanciones) \hfill \textbf{//O(N+r)} \\
        }
        ...
        i $\leftarrow$ BusquedaBinariaSanciones(k, as.kSanciones) \hfill \textbf{//O(log(N))} \\
        res $\leftarrow$ $\&$(Siguiente(Obtener(as.kSanciones[i], as.as).conMismSanciones)) \hfill \textbf{//O(1)} \\
        
    }
    \KwData{O(N + r + log(N)) = O(N) donde r es el rango para CountingSort, si hubo sanciones. O(log(N)) en llamadas siguientes.}
